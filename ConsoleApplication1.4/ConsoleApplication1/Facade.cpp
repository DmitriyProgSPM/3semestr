...
void Facade::operation()
{
	...
	do {
		...
		x = menu();
		if (x == 1) {
			l.message("Пользователь выбрал управление базой");
			y = menuBase();
			if (y == 1) {
				l.message("Пользователь выбрал состояние базы");
				f.baseState();
			}
			else if (y == 2) {
				l.message("Пользователь выбрал состояние войска");
				f.unitsState();
			}
			else if (y == 3) {
				l.message("Пользователь выбрал создание юнита");
				y = menuCreateUnit();
				Unit* unit = new Unit();
			...
			l.message("Пользователь выбрал управление юнитом");
			Unit* unit = menuChooseUnit();
			y = menuUnit();
			if (y == 1) {
				l.message("Пользователь выбрал движение юнитом");
				unit->move(menuDir());
			}
			else if(y == 2) {
				l.message("Пользователь выбрал атака юнитом");
				unit->attack(menuDir());
			}
		}
	} while (x != 3);
